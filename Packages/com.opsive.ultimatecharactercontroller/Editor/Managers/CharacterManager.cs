...
namespace Opsive.UltimateCharacterController.Editor.Managers
{
    ...
    public class CharacterManager : Manager
    {
        ...
        public override void Initialize(MainManagerWindow mainManagerWindow)
        {
            ...
            m_CharacterItemSlotWindow = EditorWindow.FindFirstObjectByType<CharacterItemSlotWindow>();
#else
...
#if UNITY_2023_1_OR_NEWER
            m_ReferenceResolverWindow = EditorWindow.FindFirstObjectByType<ReferenceResolverWindow>();
#else
        ...
        private void ShowCharacter(VisualElement container)
        {
            ...
            m_ScrollView.Add(horizontalLayout);
            ...
            var canBuild = true;
            ShowCharacterField(container, m_ScrollView, canBuild);
            if (canBuild) {
                canBuild = CheckValidCharacter(m_ScrollView, m_Character) && canBuild;
            }

            canBuild = ShowPerspective(container, m_ScrollView) && canBuild;
            if (canBuild) {
                ...
                m_ScrollView.Add(firstPersonMovementTypeField);
                ...
                m_ScrollView.Add(thirdPersonMovementTypeField);
            }

            canBuild = ShowAnimator(container, m_ScrollView, canBuild && m_Perspective == Perspective.First && m_Character == null, canBuild) && canBuild;

            if (canBuild) {
                var modelContainer = new VisualElement();
                canBuild = ShowModelOptions(container, modelContainer, canBuild);
                m_ScrollView.Add(modelContainer);
            }
            ...
            if (m_Character == null || m_CharacterModels == null) {
                canBuild = ShowFirstPerson(container, m_ScrollView, canBuild, 0);
                canBuild = ShowItemSlots(container, m_ScrollView, canBuild, 0) && canBuild;
            }
            ...
            m_ScrollView.Add(templateCharacterField);
            ...
            if (m_TemplateCharacter != null) {
                if (m_TemplateCharacter.GetComponent<UltimateCharacterLocomotion>() == null) {
                    if (canBuild) {
                        m_ScrollView.Add(new HelpBox("The template character must be an existing character.", HelpBoxMessageType.Error));
                    }
                    canBuild = false;
                } else if (m_TemplateCharacter == m_Character) {
                    if (canBuild) {
                        m_ScrollView.Add(new HelpBox("The template character cannot be the same as the character.", HelpBoxMessageType.Error));
                    }
                    canBuild = false;
                } else if (m_TemplateCharacter.GetComponent<LegacyCharacterLocomotion>() != null) {
                    if (canBuild) {
                        m_ScrollView.Add(new HelpBox("The template character cannot a legacy character.", HelpBoxMessageType.Error));
                    }
                    canBuild = false;
                }

                optionsContainer.SetEnabled(canBuild);
                m_ScrollView.Add(optionsContainer);
        ...
        private bool ShowModelOptions(VisualElement baseContainer, VisualElement container, bool canBuild)
        {
            ...
            for (int i = 0; i < m_CharacterModels.Length + 1; ++i) {
                ...
                if (canBuild && i > 0 && i < m_CharacterModels.Length && m_CharacterModels[i] != null && m_CharacterModels[i] != m_Character) {
                    if (m_CharacterModels[i].GetComponent<UltimateCharacterLocomotion>() != null) {
                        container.Add(new HelpBox("The character model cannot be an already created character.", HelpBoxMessageType.Error));
                        canBuild = false;
                    } else if (EditorUtility.IsPersistent(m_CharacterModels[i])) {
                        container.Add(new HelpBox("Please drag your character model into the scene.", HelpBoxMessageType.Error));
                        canBuild = false;
                    }
                }
        ...
        private void UpdateCharacter(VisualElement container)
        {
            ...
            if (m_Perspective != CurrentPerspective()) {
                ...
                if (m_Perspective == Perspective.Both) {
                    if (perspectiveMonitor == null) {
                        perspectiveMonitor = m_Character.AddComponent<ThirdPersonController.Character.PerspectiveMonitor>();
                        perspectiveMonitor.InvisibleMaterial = m_InvisibleShadowCaster;
                    }
            ...
            if (m_Animator != HasAnimator(m_Character)) {
                ...
                for (int i = 0; i < animatorMonitors.Length; ++i) {
                    var animator = animatorMonitors[i].GetComponent<Animator>();
                    if (animator == null || m_AnimatorControllers.Length <= i) {
                        continue;
                    }
        ...
        /// <summary>
        /// Returns the first existing object with the specified type that hasn't already been matched.
        /// </summary>
        /// <typeparam name="T">The object type.</typeparam>
        /// <param name="objects">The objects to search.</param>
        /// <param name="type">The type to match.</param>
        /// <param name="usedObjects">The objects that have already been matched.</param>
        /// <returns>The first matching object.</returns>
        private static T GetExistingObject<T>(T[] objects, Type type, HashSet<T> usedObjects) where T : class
        {
            if (objects == null) {
                return null;
            }
            ...
            for (int i = 0; i < objects.Length; ++i) {
                if (objects[i] == null || usedObjects.Contains(objects[i]) || objects[i].GetType() != type) {
                    continue;
                }
        ...
        /// <summary>
        /// Removes any extra item abilities which were previously duplicated from the template.
        /// </summary>
        /// <param name="characterLocomotion">The character locomotion containing the item abilities.</param>
        /// <param name="templateItemAbilities">The template item abilities.</param>
        /// <param name="matchedItemAbilities">The item abilities that should be preserved.</param>
        private static void RemoveExtraItemAbilities(UltimateCharacterLocomotion characterLocomotion, Character.Abilities.Items.ItemAbility[] templateItemAbilities,
            HashSet<Character.Abilities.Items.ItemAbility> matchedItemAbilities)
        {
            if (characterLocomotion.ItemAbilities == null || templateItemAbilities == null || templateItemAbilities.Length == 0 || matchedItemAbilities == null) {
                return;
            }

            var templateTypes = new HashSet<Type>();
            for (int i = 0; i < templateItemAbilities.Length; ++i) {
                if (templateItemAbilities[i] == null) {
                    continue;
                }

                templateTypes.Add(templateItemAbilities[i].GetType());
            }

            for (int i = characterLocomotion.ItemAbilities.Length - 1; i >= 0; --i) {
                var itemAbility = characterLocomotion.ItemAbilities[i];
                if (itemAbility == null || !templateTypes.Contains(itemAbility.GetType()) || matchedItemAbilities.Contains(itemAbility)) {
                    continue;
                }

                var abilityDrawer = Controls.Types.AbilityDrawers.AbilityDrawerUtility.FindAbilityDrawer(itemAbility.GetType(), true);
                if (abilityDrawer != null) {
                    abilityDrawer.AbilityRemoved(itemAbility, characterLocomotion);
                }
                AbilityBuilder.RemoveAbility(characterLocomotion, itemAbility);
            }
        }
        ...
        private void CopyTemplateCharacter()
        {
            ...
            var matchedAbilities = new HashSet<Character.Abilities.Ability>();
            if (m_CopyAbilities && templateCharacterLocomotion.Abilities != null) {
                for (int i = 0; i < templateCharacterLocomotion.Abilities.Length; ++i) {
                    var templateAbility = templateCharacterLocomotion.Abilities[i];
                    var addedAbility = GetExistingObject(characterLocomotion.Abilities, templateAbility.GetType(), matchedAbilities);
                    if (addedAbility == null) {
                        addedAbility = AbilityBuilder.AddAbility(characterLocomotion, templateAbility.GetType());
                    ...
                    matchedAbilities.Add(addedAbility);

                    toResolveTemplateObjects.Add(templateAbility);
                    toResolveTargetObjects.Add(addedAbility);
                }
            }

            var matchedItemAbilities = new HashSet<Character.Abilities.Items.ItemAbility>();
            if (m_CopyItemAbilities && templateCharacterLocomotion.ItemAbilities != null) {
                for (int i = 0; i < templateCharacterLocomotion.ItemAbilities.Length; ++i) {
                    var templateItemAbility = templateCharacterLocomotion.ItemAbilities[i];
                    var addedItemAbility = GetExistingObject(characterLocomotion.ItemAbilities, templateItemAbility.GetType(), matchedItemAbilities);
                    if (addedItemAbility == null) {
                        addedItemAbility = AbilityBuilder.AddItemAbility(characterLocomotion, templateItemAbility.GetType());
                        // Allow the ability to perform any initialization.
                    ...
                    matchedItemAbilities.Add(addedItemAbility);

                    toResolveTemplateObjects.Add(templateItemAbility);
                    toResolveTargetObjects.Add(addedItemAbility);
                }
            }
            if (m_CopyItemAbilities && templateCharacterLocomotion.ItemAbilities != null) {
                RemoveExtraItemAbilities(characterLocomotion, templateCharacterLocomotion.ItemAbilities, matchedItemAbilities);
            }

            var matchedEffects = new HashSet<Character.Effects.Effect>();
            if (m_CopyEffects && templateCharacterLocomotion.Effects != null) {
                for (int i = 0; i < templateCharacterLocomotion.Effects.Length; ++i) {
                    var templateEffect = templateCharacterLocomotion.Effects[i];
                    var addedEffect = GetExistingObject(characterLocomotion.Effects, templateEffect.GetType(), matchedEffects);
                    if (addedEffect == null) {
                        addedEffect = EffectBuilder.AddEffect(characterLocomotion, templateEffect.GetType());
                    }
                    matchedEffects.Add(addedEffect);

                    toResolveTemplateObjects.Add(templateEffect);
            ...
            if (conflictingObjects.Count > 0) {
                ...
                m_ReferenceResolverWindow.Initialize(conflictingObjects, m_TemplateCharacter, "character", RemoveDuplicateHealthHitboxes);
            }
        }

        /// <summary>
        /// Removes duplicate hitboxes that resolve to the same collider on the new character.
        /// </summary>
        private void RemoveDuplicateHealthHitboxes()
        {
            var healthComponents = m_Character.GetComponents<Traits.Health>();
            for (int i = 0; i < healthComponents.Length; ++i) {
                var hitboxes = healthComponents[i].Hitboxes;
                if (hitboxes == null || hitboxes.Length < 2) {
                    continue;
                }

                // Multiple template hierarchies can resolve to the same target collider. Keep the first hitbox for that collider.
                var uniqueHitboxes = new List<Traits.Hitbox>(hitboxes.Length);
                var colliders = new HashSet<Collider>();
                var removedDuplicate = false;
                for (int j = 0; j < hitboxes.Length; ++j) {
                    var hitbox = hitboxes[j];
                    if (hitbox == null || hitbox.Collider == null || colliders.Add(hitbox.Collider)) {
                        uniqueHitboxes.Add(hitbox);
                        continue;
                    }
                ...
                healthComponents[i].Hitboxes = uniqueHitboxes.ToArray();
                Shared.Editor.Utility.EditorUtility.SetDirty(healthComponents[i]);
            }
        }
    }
}
