...
namespace Opsive.UltimateCharacterController.Inventory
{
    ...
    public class ItemSetGroup
    {
        ...
        protected ItemSet m_ActiveItemSet;
        protected ItemSet m_NextItemSet;
        ...
        public virtual CategoryBase SerializedItemCategory
        {
            get { return m_SerializedItemCategory; }
            set { m_SerializedItemCategory = value; }
        }

        public virtual IItemCategoryIdentifier ItemCategory
        {
            get { return m_ItemCategoryIdentifier; }
            set { m_ItemCategoryIdentifier = value; }
        }

        public ItemSetRuleBase[] StartingItemSetRules
        {
            get { return m_StartingItemSetRules; }
            set { m_StartingItemSetRules = value; }
        }
        ...
        public int ActiveItemSetIndex { get => m_ActiveItemSet?.Index ?? -1; set { UpdateActiveItemSet(value); } }

        public IReadOnlyList<ItemSetStateInfo> ItemSetStateList => m_ItemSetStateList;

        public int NextItemSetIndex { get => m_NextItemSet?.Index ?? -1; set { UpdateNextItemSet(value); } }
        ...
        public void Initialize(GameObject gameObject, ItemSetManagerBase itemSetManager)
        {
            if (m_ItemCategoryIdentifier == null) {
                ...
                if (m_SerializedItemCategory != null && m_ItemCategoryIdentifier == null) {
                    Debug.LogError(
                        $"The serialized ItemCategory {m_SerializedItemCategory} isn't of type {typeof(IItemCategoryIdentifier)}.",
                        gameObject);
                }

                if (m_ItemCategoryIdentifier == null) {
                    Debug.LogError(
                        $"The Item Set Group has a null ItemCategory on {gameObject}, A category must be assigned.",
                        gameObject);
            ...
            if (m_EquipUnequip == null) {
                ...
                if (m_EquipUnequip == null && Application.isPlaying) {
                    if (firstEquipUnequipAbility == null) {
                        Debug.LogError(
                            $"No Equip/Unequip ability was foundmatching the ItemSetGroup category ID '{CategoryID}'. Please add at least one Equip/Unequip ability.");
                    } else {
                        Debug.LogWarning(
                            $"No Equip/Unequip ability was found matching the ItemSetGroup category ID '{CategoryID}'. " +
        ...
        {
            return m_DefaultItemSetIndex == (m_ActiveItemSet?.Index ?? -1);
        }
        ...
        public bool IsItemSetValid(ItemSet itemSet, bool checkIfCanSwitchTo, int allowedSlotsMask = -1)
        {
            ...
            for (int i = 0; i < itemSet.ItemIdentifiers.Length; ++i) {
                ...
                if (allowedSlotsMask != -1 && DefaultItemSetIndex != itemSet.Index &&
                    !MathUtility.InLayerMask(i, allowedSlotsMask)) {
                    return false;
                }
        ...
        public virtual void UpdateItemSets(List<CharacterItem>[] characterItemsBySlot)
        {
            ...
            var pooledItemSetsToDestroy = GenericObjectPool.Get<List<ItemSet>>();
            pooledItemCategorySetList.Clear();
            pooledItemSets.Clear();
            pooledItemSetsToDestroy.Clear();
            ...
            EventHandler.ExecuteEvent<ItemSetGroup, List<ItemSetStateInfo>>(m_GameObject, "OnItemSetGroupWillUpdate",
                this, m_ItemSetStateList);
            ...
            var temporaryAssignedActiveItemSetPriority = -1; // Increases for each match in the item set.
            var temporaryAssignedNextItemSetPriority = -1; // Increases for each match in the item set.
            ...
            for (int i = 0; i < m_ItemSetStateList.Count; i++) {
                ...
                switch (itemSetStateInfo.State) {
                    case ItemSetStateInfo.SetState.Keep:
                    {
                        ...
                        CheckActiveNextItemSet(itemSet, activeItemSet, nextItemSet, setCount,
                            ref temporaryAssignedActiveItemSet, ref temporaryAssignedNextItemSet,
                            ref temporaryAssignedActiveItemSetPriority, ref temporaryAssignedNextItemSetPriority);
                    ...
                    case ItemSetStateInfo.SetState.Remove:
                    {
                        ...
                        pooledItemSetsToDestroy.Add(itemSet);
                        break;
                    }
                    default:
                    {
            ...
            foreach (var itemSetToDestroy in pooledItemSetsToDestroy) {
                if (activeItemSet == itemSetToDestroy) {
                    SetStateForItemSet(itemSetToDestroy, null);
                }

                DestroyItemSet(itemSetToDestroy);
            }

            var newActiveIndex = IsItemSetValid(temporaryAssignedActiveItemSet, false, -1)
                ? temporaryAssignedActiveItemSet
                : -1;
            var newNextIndex = IsItemSetValid(temporaryAssignedNextItemSet, false, -1)
                ? temporaryAssignedNextItemSet
                : -1;
            ...
            GenericObjectPool.Return(pooledItemSetsToDestroy);
        ...
        private void CheckActiveNextItemSet(ItemSet itemSet, ItemSet activeItemSet, ItemSet nextItemSet, int setIndex,
            ref int temporaryAssignedActiveItemSet, ref int temporaryAssignedNextItemSet,
            ref int temporaryAssignedActiveItemSetPriority, ref int temporaryAssignedNextItemSetPriority)
        {
            if (temporaryAssignedActiveItemSetPriority != SlotCount && (activeItemSet == itemSet) && IsItemSetValid(setIndex, false, -1)) {
                temporaryAssignedActiveItemSet = setIndex;
                temporaryAssignedActiveItemSetPriority = SlotCount;
            }

            if (temporaryAssignedNextItemSetPriority != SlotCount && (nextItemSet == itemSet) && IsItemSetValid(setIndex, false, -1)) {
                temporaryAssignedNextItemSet = setIndex;
                temporaryAssignedNextItemSetPriority = SlotCount;
            }

            if (itemSet == null || nextItemSet == null || nextItemSet.ItemIdentifiers == null) {
                return;
            }

            if (nextItemSet != null && temporaryAssignedNextItemSetPriority < SlotCount) {
                var matchCount = -1;
                foreach (var nextItemIdentifier in nextItemSet.ItemIdentifiers) {
                    matchCount += itemSet.ItemIdentifiers.Contains(nextItemIdentifier) ? 1 : 0;
                }

                if (temporaryAssignedNextItemSetPriority < matchCount && IsItemSetValid(setIndex, false, -1)) {
                    temporaryAssignedNextItemSet = setIndex;
                    temporaryAssignedNextItemSetPriority = matchCount;
                }
            }
        }
        ...
        public void UpdateNextItemSet(int itemSetIndex)
        {
            ...
            if (itemSetIndex < 0 || itemSetIndex >= m_ItemSetList.Count) {
                ...
                m_NextItemSet = m_ItemSetList[itemSetIndex];
            }

            EventHandler.ExecuteEvent<int, int, int>(m_GameObject, "OnItemSetManagerUpdateNextItemSet", GroupIndex,
                prevItemSetIndex, itemSetIndex);
        }
        ...
        public virtual void UpdateActiveItemSet(int newActiveIndex)
        {
            ...
            ItemSet previousItemSet = m_ActiveItemSet;
            ...
            if (newActiveIndex != -1) {
                newItemSet = ItemSetList[newActiveIndex];
                m_ActiveItemSet = newItemSet;
                newItemSet.Active = true;
#if UNITY_EDITOR
                if (newItemSet.Enabled == false) {
                    Debug.LogError(
                        $"The Item Set [{newActiveIndex}]{newItemSet.State} in the group [{GroupIndex}]{ItemCategory} was set active when it is actually disabled. This should not happen.");
                }
#endif
            }
            ...
            EventHandler.ExecuteEvent<int, ItemSet, ItemSet>(m_GameObject, "OnActiveItemSetChange", GroupIndex,
                previousItemSet, newItemSet);
            EventHandler.ExecuteEvent<int, int>(m_GameObject, "OnItemSetManagerUpdateItemSet", GroupIndex,
                newActiveIndex);
        }
        ...
        {
            for (int i = 0; i < m_ItemSetList.Count; i++) {
                if (ItemSetList[i].State == itemSetName) {
                    return ItemSetList[i];
                }
            }
        ...
        public int FindClosestMatchingValidItemSet(int itemSetIndex, bool useDefaultIfNoMatch,
            bool matchThisItemSetIfValid)
        {
            if (itemSetIndex < 0 || itemSetIndex >= m_ItemSetList.Count) {
                return useDefaultIfNoMatch ? DefaultItemSetIndex : -1;
            }

            var thisItemSet = m_ItemSetList[itemSetIndex];
            if (matchThisItemSetIfValid && thisItemSet.IsValid) {
                return itemSetIndex;
            }

            var bestMatch = useDefaultIfNoMatch ? DefaultItemSetIndex : -1;
            var bestMatchPriority = 0;
            var bestIndexMatch = 0;

            for (var i = 0; i < m_ItemSetList.Count; i++) {
                var itemSet = m_ItemSetList[i];
                if (itemSet == thisItemSet || !itemSet.IsValid) {
                    continue;
                }

                int sharedCount = 0;
                int indexMatchCount = 0;
                for (int j = 0; j < Math.Min(thisItemSet.ItemIdentifiers.Length, itemSet.ItemIdentifiers.Length); j++) {
                    var idA = thisItemSet.ItemIdentifiers[j];
                    var idB = itemSet.ItemIdentifiers[j];
                    if (idA != null) {
                        if (idB != null && idA.Equals(idB)) {
                            sharedCount++;
                            indexMatchCount++;
                        } else if (itemSet.ItemIdentifiers.Contains(idA)) {
                            sharedCount++;
                        }
                    }
                }

                if (sharedCount > bestMatchPriority ||
                    (sharedCount == bestMatchPriority && indexMatchCount > bestIndexMatch)) {
                    bestMatchPriority = sharedCount;
                    bestIndexMatch = indexMatchCount;
                    bestMatch = i;
                }
            }
