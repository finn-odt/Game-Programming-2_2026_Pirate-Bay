...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class CharacterIK : CharacterIKBase
    {
        ...
        [SerializeField, HideInInspector] protected float m_HandWeight = 1;
        [SerializeField, HideInInspector] protected bool m_IndividualHandWeightsInitialized;
        [Shared.Utility.InspectorFoldout("Hands")]
        [Tooltip("Determines how much weight is applied to the left hand when looking at the target (0-1).")]
        [Range(0, 1)] [SerializeField] protected float m_LeftHandWeight = 1;
        [Tooltip("Determines how much weight is applied to the right hand when looking at the target (0-1).")]
        [Range(0, 1)] [SerializeField] protected float m_RightHandWeight = 1;
        [Tooltip("Determines how much weight is applied to the left elbow when looking at the target (0-1).")]
        [Range(0, 1)] [SerializeField] protected float m_LeftElbowWeight = 1;
        [Tooltip("Determines how much weight is applied to the right elbow when looking at the target (0-1).")]
        [Range(0, 1)] [SerializeField] protected float m_RightElbowWeight = 1;
        ...
        {
            get { InitializeIndividualHandWeights(); return m_HandWeight; }
            set {
                m_HandWeight = value;
                m_LeftHandWeight = m_RightHandWeight = m_LeftElbowWeight = m_RightElbowWeight = value;
                m_IndividualHandWeightsInitialized = true;
            }
        }
        public float LeftHandWeight { get { InitializeIndividualHandWeights(); return m_LeftHandWeight; } set { InitializeIndividualHandWeights(); m_LeftHandWeight = value; UpdateSharedHandWeight(); } }
        public float RightHandWeight { get { InitializeIndividualHandWeights(); return m_RightHandWeight; } set { InitializeIndividualHandWeights(); m_RightHandWeight = value; UpdateSharedHandWeight(); } }
        public float LeftElbowWeight { get { InitializeIndividualHandWeights(); return m_LeftElbowWeight; } set { InitializeIndividualHandWeights(); m_LeftElbowWeight = value; UpdateSharedHandWeight(); } }
        public float RightElbowWeight { get { InitializeIndividualHandWeights(); return m_RightElbowWeight; } set { InitializeIndividualHandWeights(); m_RightElbowWeight = value; UpdateSharedHandWeight(); } }
        ...
        private float[] m_ElbowIKWeight = new float[2];
        ...
        {
            base.Awake();
            InitializeIndividualHandWeights();
        ...
        /// <summary>
        /// Ensure older serialized hand weights are migrated in the editor.
        /// </summary>
        private void OnValidate()
        {
            var initialized = m_IndividualHandWeightsInitialized;
            InitializeIndividualHandWeights();
            if (initialized) {
                UpdateSharedHandWeight();
            }
        }
#endif

        /// <summary>
        /// Initializes the individual hand and elbow weights from the legacy shared weight.
        /// </summary>
        private void InitializeIndividualHandWeights()
        {
            if (m_IndividualHandWeightsInitialized) {
                return;
            }

            m_LeftHandWeight = m_RightHandWeight = m_LeftElbowWeight = m_RightElbowWeight = m_HandWeight;
            m_IndividualHandWeightsInitialized = true;
        }

        /// <summary>
        /// Updates the legacy shared hand weight from the individual weights.
        /// </summary>
        private void UpdateSharedHandWeight()
        {
            m_HandWeight = (m_LeftHandWeight + m_RightHandWeight + m_LeftElbowWeight + m_RightElbowWeight) / 4;
        }

        /// <summary>
        /// Returns the hand IK weight for the specified side.
        /// </summary>
        /// <param name="leftHand">Is the weight for the left hand?</param>
        /// <returns>The hand IK weight for the specified side.</returns>
        private float GetHandWeight(bool leftHand)
        {
            InitializeIndividualHandWeights();
            return Mathf.Clamp01(leftHand ? m_LeftHandWeight : m_RightHandWeight);
        }

        /// <summary>
        /// Returns the elbow IK hint weight for the specified side.
        /// </summary>
        /// <param name="leftElbow">Is the weight for the left elbow?</param>
        /// <returns>The elbow IK hint weight for the specified side.</returns>
        private float GetElbowWeight(bool leftElbow)
        {
            InitializeIndividualHandWeights();
            return Mathf.Clamp01(leftElbow ? m_LeftElbowWeight : m_RightElbowWeight);
        }
        ...
        private void RotateHands()
        {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                // is being used.
                var targetWeight = 0f;
                if (ikTarget != null || m_Aiming || m_ItemInUse || firstPersonPerspective || m_OnUpdateIKRotation != null) {
                    targetWeight = GetHandWeight(i == 0);
                }
        ...
        private void PositionHands()
        {
            for (int i = 0; i < 2; ++i) {
                ...
                if (ikTarget != null || m_Aiming || m_ItemInUse || firstPersonPerspective || m_UpperArmWeight > 0 || m_OnUpdateIKPosition != null) {
                    targetWeight = GetHandWeight(i == 0);
                    targetElbowWeight = hintTarget != null ? GetElbowWeight(i == 0) : 0;
                }
                ...
                m_ElbowIKWeight[i] = Mathf.Clamp01(m_ImmediatePosition ? targetElbowWeight :
                                                Mathf.MoveTowards(m_ElbowIKWeight[i], targetElbowWeight, m_HandAdjustmentSpeed * TimeUtility.DeltaTime));
                ...
                if (hintTarget != null && m_ElbowIKWeight[i] > 0) {
                ...
                m_Animator.SetIKHintPositionWeight(hintGoal, hintTarget != null ? m_ElbowIKWeight[i] : 0);
            }
        }
