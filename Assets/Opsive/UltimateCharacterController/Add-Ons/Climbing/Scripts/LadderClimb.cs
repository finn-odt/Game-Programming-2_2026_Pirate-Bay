...
namespace Opsive.UltimateCharacterController.AddOns.Climbing
{
    ...
    public class LadderClimb : TraversalClimb
    {
        ...
        private Quaternion m_LadderTargetWorldRotation = Quaternion.identity;
        private Quaternion m_PreLadderWorldRotation = Quaternion.identity;
        private bool m_HasLadderRotation;
        ...
        protected override void AbilityStarted()
        {
            ...
            m_PreLadderWorldRotation = m_CharacterLocomotion.Rotation;
            m_LadderTargetWorldRotation = Quaternion.LookRotation(
                m_Ladder.GetLookDirection(m_CharacterLocomotion),
                m_Ladder.transform.up
            );
            m_HasLadderRotation = true;
        ...
        public override void UpdateRotation()
        {
            ...
            if (m_HasLadderRotation) {
                // converge against same rotation
                m_CharacterLocomotion.DesiredRotation = Quaternion.Inverse(m_CharacterLocomotion.Rotation) * m_LadderTargetWorldRotation;
            }
        }
        ...
        protected override void AbilityStopped(bool force)
        {
            ...
            if (m_HasLadderRotation) {
                RestoreWorldUpRotationImmediate();
                m_HasLadderRotation = false;
            }
        ...
        private void RestoreWorldUpRotationImmediate()
        {
            // Keep the character's current facing direction, but remove ladder roll/pitch.
            var forward = m_CharacterLocomotion.Rotation * Vector3.forward;
            forward = Vector3.ProjectOnPlane(forward, Vector3.up);

            if (forward.sqrMagnitude < 0.001f) {
                forward = Vector3.ProjectOnPlane(m_PreLadderWorldRotation * Vector3.forward, Vector3.up);
            }
            ...
            var targetWorldRotation = Quaternion.LookRotation(forward.normalized, Vector3.up);

            // Important: apply the rotation immediately. Do not only set DesiredRotation here.
            m_CharacterLocomotion.SetPositionAndRotation(
