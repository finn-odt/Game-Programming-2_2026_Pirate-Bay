...
public class OcclusionCameraController : MonoBehaviour
{
    public static OcclusionCameraController Instance { get; private set; }
    
    
    [SerializeField, LabelText("Normal Gameplay Camera")] private Camera gameplayCam;
    public Camera GameplayCamera => gameplayCam;
    ...
    private void Awake()
    {
        ...
        if(gameplayCam == null)
            gameplayCam = GetComponent<Camera>();
    ...
    {
        // TODO: anpassen für mehr Kameras
        return gameplayCam;
    }
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Vector3 playerPos = player.Position + clearViewOffset;
    ...
    {
        Vector3 playerPos = player.Position + clearViewOffset;  // get position (with offset for body/head)
        ...
        for(int i = 0; i < hits.Length; i++)
        {
            ...
            foreach (Renderer renderer in renderers)
            {
                ...
                //Debug.Log($"Layer of occluded object '{obj.name}': {obj.layer} aka {((LayerId)obj.layer).ToString()}");
